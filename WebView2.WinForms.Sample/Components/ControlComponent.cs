...
using MtrDev.WebView2.WinForms.Sample.Controls;
using System;
    ...
    public class ControlComponent
    {
        ...
        private void WebView2MoveFocusRequested(object sender, Wrapper.MoveFocusRequestedEventArgs e)
        {
            if (!_parent.AutoTabHandle)
            {
                ...
                {
                    _parent.SelectNextControl(_webView2, true, true, true, true);
                }
                else if (reason == WEBVIEW2_MOVE_FOCUS_REASON.WEBVIEW2_MOVE_FOCUS_REASON_PREVIOUS)
                {
                    _parent.SelectNextControl(_webView2, false, true, true, true);
                }
                e.Handled = true;
            }
        }
